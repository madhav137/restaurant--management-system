...
PreparedStatement pst = null;
	...
	public UpdateRoom() throws SQLException {
		...
		contentPane.setLayout(null);
                ...
                ImageIcon i1  = new ImageIcon(ClassLoader.getSystemResource("restaurant/management/system/icons/mm-3.jpg"));
                Image i3 = i1.getImage().getScaledInstance(400, 400,Image.SCALE_DEFAULT);
                ImageIcon i2 = new ImageIcon(i3);
                JLabel l1 = new JLabel(i2);
                l1.setBounds(280,80,600,250);
                add(l1);
		
		JLabel lblUpdateRoomStatus = new JLabel("UPDATE TABLE STATUS");
		lblUpdateRoomStatus.setFont(new Font("Tahoma", Font.PLAIN, 15));
		...
		JLabel lblAvailability = new JLabel("AVAILABLITY:");
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				conn c = new conn();
                                ...
                                while(rs1.next()){
                                    txt_Room.setText(rs1.getString("tables"));    
                                }
                            }catch(Exception ee){}
                            try{
                                conn c  = new conn();
                                ResultSet rs2 = c.s.executeQuery("select * from room where table_number = "+txt_Room.getText());
                                while(rs2.next()){
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) throws NumberFormatException {
				...
				try{
                                    conn c = new conn();
                                    String str = "update room set available = '"+txt_Ava.getText()+"',status='"+txt_Status.getText()+"' where table_number = "+txt_Room.getText();
		...
		JLabel lblRoomId = new JLabel("TABLE NUMBER:");
